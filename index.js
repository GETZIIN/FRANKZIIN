		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD getziin*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] getziin? Este é um recurso especial para o getziin❌',
					ownerB: '[❗] GETZIIN? Este é um recurso especial para o getziin ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Frankziin. Registre-se para fazer amizade com o frankziin por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Getziin|16\n\n──「 FRANKZIIN 」──`,
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						if (isAntiRacismo) return reply('O modo antiracismo já está ativo burro')
				...
				case 'modonsfw':
					...
					if (args.length < 1) return reply('?')
					if ((args[0]) === 'on') {
						if (isNsfw) return reply('O modo nsfw já está ativo burro')
						nsfw.push(from)
...
//@darkYT					
    ...
    break
				...
				case 'semoji':
					if (args.length < 1) return reply('onde está o emojis cavalo?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'textblue':
					...
					break
					case 'vipmenu':
					if (!isPremium) return reply('Você não é um Membro Premium,apenas um menbro comum 🤣🤣, entre em contato com o proprietário ou digite *#daftarvip* para comprar acesso Premium!' ,text, { quoted: mek })
                ...
                case 'wolflogo':
                      ...
                      break
                case 'lionlogo':
                      if (args.length < 1) return reply('Cadê o texto cavalo?')
                      gh = body.slice(9)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
		...
		case 'bpfont':
			...
			break 
				...
				case 'del':
				case 'd':  
					if (!isPremium) return reply('Você não é um Membro Premium apenas um membro comum🤣🤣, entre em contato com o proprietário ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
...
break
			    ...
			    case 'lovemake':
              	    if (args.length < 1) return reply('Onde está o texto, irmão??')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'no maximo 10 caracter', text, {quoted: mek})
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS FRANKZIIN 👑* 」\n\`\`\``
					no = 0
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o meu dono, qm e vc,ata um mumbro comum?')
...
// recursos anime, random @GETZIIN domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o GETZIIN ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT GETZIIN ✨* 」\n*│+ wa.me/554599174965*╰──────*「 *FRANKZIIN* 」*────*\n\n*_SE QUER SER ADMIN DO FRANKZIIN_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/554599174965/*╰──────*「 * GETZIIN * 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO FRANKZIIN_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*ENTRE EM CTT COM DONO DO BOT*'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* GETZIIN*WPP:* wa.me/+554599174965\n*INSTA:* @gustavo_65_p\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
